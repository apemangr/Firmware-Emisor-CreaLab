...
int         main(void)
{
    ...
    if (impresion_log)
    {
        for (int loop_med = 0; loop_med <= 10; loop_med++)
        {
            ...
            {
                NRF_LOG_RAW_INFO("fecha %d/%d/%d ", history_record.day, history_record.month,
                                 history_record.year);
                NRF_LOG_FLUSH();
                NRF_LOG_RAW_INFO("%d:%d:%d ", history_record.hour, history_record.minute,
                                 history_record.second);
                NRF_LOG_FLUSH();
                NRF_LOG_RAW_INFO("historia %d:- %d/%d/%d/%d \r\n", loop_med,
                                 history_record.Contador, history_record.V1, history_record.V2,
                                 history_record.battery);
                NRF_LOG_FLUSH();
            }
            nrf_delay_ms(20);
        }
    ...
    {
        Step_of_resistor = 1.3000f;
    }
    if (Tipo_dispositivo == 0x14)
    {
        Step_of_resistor = 1.3000f;
    } // Con alerta interior Perno.
    if (Tipo_dispositivo == 0x15)
    {
        Step_of_resistor = 1.5000f;
    } // Sensor 100 divisiones de 1.5mm
    if (Tipo_dispositivo == 0x16)
    {
        Step_of_resistor = 4.0000f;
    ...
    if (Flash_array.Enable_Custom_mac == 0x01 &&
        Flash_array.mac_custom[0] != 0x00) // Si hay que ocupar la mac custom y el dato no es cero.
    {
    ...
    nrf_delay_ms(100);
    Reset_Line_Step(13); // carga condensadores
    gap_params_init();
    Reset_Line_Step(14);
    gatt_init();
    Reset_Line_Step(15);
    ...
    for (;;)
    {
        ...
        if (Wakeup_by_Timer)
        {
            ...
            NRF_LOG_RAW_INFO("Starting by RTC N: %d y contador %d  ", loop_timer, contador);
        ...
        if (Wakeup_by_button)
        {
            ...
            Device_on        = true;
            NRF_LOG_RAW_INFO("Starting by button N� %d y contador %d  ", loop_button, contador);
            NRF_LOG_RAW_INFO("%02i/%02i/%02i - ", t.year, t.month, t.date);
            NRF_LOG_RAW_INFO("%02i:%02i:%02i:%02i \r\n", t.hour, t.minute, t.second, t.msecond);
            NRF_LOG_FLUSH();
            advertising_init();
