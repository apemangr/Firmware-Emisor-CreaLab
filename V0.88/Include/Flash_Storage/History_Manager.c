...
ret_code_t history_add_record(const store_History* record)
{
    ...
    if (cache_slot >= HISTORY_CACHE_SIZE)
    {
        ...
        {
            NRF_LOG_DEBUG("Cache lleno, reusando slot %d (estado: %d)", cache_slot, g_history_manager.cache[cache_slot].state);
        }
    }
    
    // Validar dirección de flash antes de escribir
    uint32_t flash_addr = HISTORY_FLASH_START_ADDR + (g_history_manager.next_flash_index * sizeof(store_History));
    ...
    NRF_LOG_RAW_INFO("Nuevo registro añadido al historial. Total: %d, Flash index: %d", 
                  g_history_manager.total_records, g_history_manager.cache[cache_slot].flash_index);
    return NRF_SUCCESS;
}
...
static ret_code_t history_write_cache_to_flash(uint8_t cache_index)
{
    ...
    {
        NRF_LOG_WARNING("Cache slot %d no está sucio, estado: %d", cache_index, g_history_manager.cache[cache_index].state);
    ...
    NRF_LOG_DEBUG("Flash addr calculada: 0x%x, flash_index: %d, tamaño: %d", 
                  flash_addr, flash_index, sizeof(store_History));
    
    // Validaciones exhaustivas
    if (flash_addr < HISTORY_FLASH_START_ADDR)
    {
        NRF_LOG_ERROR("Dirección flash menor al inicio: 0x%x < 0x%x", flash_addr, HISTORY_FLASH_START_ADDR);
        return NRF_ERROR_INVALID_ADDR;
    }
    
    if (flash_addr + sizeof(store_History) > HISTORY_FLASH_END_ADDR)
    {
        NRF_LOG_ERROR("Dirección flash excede el final: 0x%x + %d > 0x%x", 
                      flash_addr, sizeof(store_History), HISTORY_FLASH_END_ADDR);
        return NRF_ERROR_NO_MEM;
    }
    ...
    NRF_LOG_DEBUG("Page addr: 0x%x, record_in_page: %d", page_addr, record_in_page);
    
    if (record_in_page == 0)
    {
        NRF_LOG_INFO("Borrando página en dirección 0x%x", page_addr);
    ...
    NRF_LOG_INFO("Escribiendo %d bytes a flash en dirección 0x%x", sizeof(store_History), flash_addr);
    
    ret_code_t rc = nrf_fstorage_write(&history_fstorage, 
                                       flash_addr,
                                       &g_history_manager.cache[cache_index].record,
                                       sizeof(store_History),
                                       NULL);
...
static ret_code_t history_read_from_flash(uint16_t flash_index, store_History* record)
{
    ...
    ret_code_t rc = nrf_fstorage_read(&history_fstorage,
                                      flash_addr,
                                      record,
                                      sizeof(store_History));
...
static void history_update_cache_lru(uint8_t cache_index)
{
    ...
    if (cache_index < HISTORY_CACHE_SIZE - 1)
    {
        ...
        memmove(&g_history_manager.cache[cache_index], 
                &g_history_manager.cache[cache_index + 1],
