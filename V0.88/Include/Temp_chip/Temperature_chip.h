...
uint16_t GET_MEASURE_SENSOR_1(nrf_saadc_input_t Pin_Sensor, nrf_saadc_gain_t Gain) {
  ...
  int T_med = 10;
  uint16_t med[20];
  nrf_delay_ms(30); /// IMPORTANTE NO BORRAR CON ESTE COMANDO SE GENERA PAUSA PARA MEDICIONES CORRECTAS ********* <------------

  for (loop_med = 0; loop_med < T_med; loop_med++) {
    nrf_delay_ms(10); // Agregar *********************************** 0.84 por ver si esta saltandose la medicion **********
...
uint16_t GET_MEASURE_SENSOR(nrf_saadc_input_t Pin_Sensor, nrf_saadc_gain_t Gain) {
 ...
 // nrf_delay_ms(30);
  ...
  DWT->CYCCNT = 0;
  while (nrf_drv_saadc_is_busy()) { // while evita que emisor se quede pegado en la instruccion
    if (DWT->CYCCNT > 6418000) {
