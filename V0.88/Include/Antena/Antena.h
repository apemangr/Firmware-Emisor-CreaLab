...
void Next_Transmition(void)
{
    ...
    if (Tipo_Envio == Values)
    {
        ...
        NRF_LOG_RAW_INFO("V1: %d, V2: %d, Contador: %d\r\n", last_record.V1, last_record.V2,
                         last_record.Contador);
        index        = position;
        Next_Sending = false;
        send_data_Nus(index, data_array);
        Tipo_Envio = 0;
...
static void advertising_init(void)
{
    ...
    if (History_Position == 0)
    {
        ...
        bool fecha_valida = (t.year >= 2024 && t.year <= 2030) && (t.month >= 1 && t.month <= 12) &&
                            (t.date >= 1 && t.date <= 31) && (t.hour >= 0 && t.hour <= 23) &&
    ...
    if (History_Position != 0)
    {
        ...
        */
        ...
        if (Another_Value)
        {
            ...
            // Validar fecha y hora
            bool fecha_valida =
                (t.year >= 2024 && t.year <= 2030) && (t.month >= 1 && t.month <= 12) &&
                (t.date >= 1 && t.date <= 31) && (t.hour >= 0 && t.hour <= 23) &&
                (t.minute >= 0 && t.minute <= 59) && (t.second >= 0 && t.second <= 59);

            if (!fecha_valida)
            {
                // Usar valores de respaldo desde Flash_array
                t.date   = Flash_array.date;
                t.month  = Flash_array.month;
                t.year   = Flash_array.year;
                t.hour   = Flash_array.hour;
    ...
    if (impresion_log)
    {
        for (int loop_med = 0; loop_med <= (History_Position); loop_med++)
        {
            ...
            if (rc == NRF_SUCCESS)
            {
                NRF_LOG_RAW_INFO("historia %d: - fecha %d/%d/%d ", loop_med, log_record.day,
                                 log_record.month, log_record.year);
                NRF_LOG_FLUSH();
                NRF_LOG_RAW_INFO("%d:%d:%d ", log_record.hour, log_record.minute,
                                 log_record.second);
                NRF_LOG_FLUSH();
                NRF_LOG_RAW_INFO("Cont: %d :  %d/%d/%d \r\n", log_record.Contador, log_record.V1,
                                 log_record.V2, log_record.battery);
                NRF_LOG_FLUSH();
            }
            else
            {
