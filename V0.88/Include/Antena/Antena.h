  ...
  if (p_evt->type == BLE_NUS_EVT_RX_DATA)
  {
    ...
    if ((p_evt->params.rx_data.p_data[0] == '0') && (p_evt->params.rx_data.p_data[1] == '2'))
    {
      ...
      sleep_in_time_ticker = (uint32_t)(((p_evt->params.rx_data.p_data[2] - 0x30) * 10000) +
                                        ...
                                        ((p_evt->params.rx_data.p_data[5] - 0x30) * 10) +
                                        ((p_evt->params.rx_data.p_data[6] - 0x30) * 1)) *
                             10; // de segundos se pasa a ticker de 0.1 segundos, se multiplica por 10
      NRF_LOG_RAW_INFO("Tiempo de dormido (02) a %i\r\n", sleep_in_time_ticker);
      NRF_LOG_FLUSH();
      if (sleep_in_time_ticker >= 20 && sleep_in_time_ticker <= 144000) // Permite dormido entre 1 segundo y 4 horas
      {
    ...
    if ((p_evt->params.rx_data.p_data[0] == '0') && (p_evt->params.rx_data.p_data[1] == '3'))
    {
      ...
      APP_ADV_DURATION = (uint32_t)(((p_evt->params.rx_data.p_data[2] - 0x30) * 10000) +
                                    ...
                                    ((p_evt->params.rx_data.p_data[5] - 0x30) * 10) +
                                    ((p_evt->params.rx_data.p_data[6] - 0x30) * 1)) *
                         100; // de segundos se pasa a ticker de 0.1 segundos, se multiplica por 10
      NRF_LOG_RAW_INFO("Comando 03 recepcionado : Modificando tiempo de adv a %i\r\n", APP_ADV_DURATION);
      NRF_LOG_FLUSH();

      if (APP_ADV_DURATION >= 100 && APP_ADV_DURATION <= 6000) // Permite adverting entre 1 segundo y 60 segundos
      {
    ...
    if ((p_evt->params.rx_data.p_data[0] == '0') && (p_evt->params.rx_data.p_data[1] == '4'))
    {
      ...
      contador = (uint32_t)(((p_evt->params.rx_data.p_data[2] - 0x30) * 10000) +
                            ...
                            ((p_evt->params.rx_data.p_data[4] - 0x30) * 100) +
                            ((p_evt->params.rx_data.p_data[5] - 0x30) * 10) +
                            ((p_evt->params.rx_data.p_data[6] - 0x30) * 1)); // de segundos se pasa a ticker de 0.1 segundos, se multiplica por 10
    ...
    if ((p_evt->params.rx_data.p_data[0] == '0') && (p_evt->params.rx_data.p_data[1] == '7'))
    {
      ...
      //      snprintf(data_arr, sizeof data_arr, "%s/%s/%s %s:%s:%s", t.date, t.month, t.year,t.hour,t.minute,(char t.second));
      uint16_t length = 19;
...
static void advertising_init(void)
{
  ...
  nrf_delay_ms(500);
  Battery_level = 0;
  Battery_level = GET_MEASURE_SENSOR(ADC_Battery, Gain_Battery); // Circuito_List[0]  NRF_SAADC_INPUT_AIN2
  ...
  NRF_LOG_FLUSH();
  uint16_t Pista2 = GET_MEASURE_SENSOR(Circuito_2, Gain_Circuit); // Circuito_List[0]  NRF_SAADC_INPUT_AIN2
  if (Pista2 > 0xf339)
    Pista2 = 0x0;
  ...
  if (impresion_log)
  {
    for (int loop_med = 0; loop_med <= (History_Position + 1); loop_med++)
    {
      ...
      NRF_LOG_FLUSH();
      NRF_LOG_RAW_INFO("Cont: %d :  %d/%d/%d \r\n", Flash_array.history[loop_med].Contador, Flash_array.history[loop_med].V1, Flash_array.history[loop_med].V2, Flash_array.history[loop_med].battery);
      NRF_LOG_FLUSH();
      nrf_delay_ms(20);
    }
  }
  ...
  m_beacon_info[12] = Flash_array.total_reset[3]; // Imprimiremos momentaneamente la cantidad de reset del equipo

  // Largo del corte esta en mm, en el sensor la division es de 2 mm.
  // al cortarse todas las resistencias quedan 2 de seguridad, pero el sensor indicará igual mente la misma medida
  // si las ultimas dos resistencias se cortan el sensor envia un 255
  ...
  manuf_specific_data.company_identifier = 0x2233; // se escribe al revez ... numero de compañia 3322
