...
void pa_assist(uint32_t gpio_pa_pin, uint32_t gpio_lna_pin) {
  ...
  opt.common_opt.pa_lna.ppi_ch_id_clr =
      ppi_set_ch; // PPI channel for pin clearing
  opt.common_opt.pa_lna.ppi_ch_id_set =
      ppi_clr_ch; // PPI channel for pin setting
  // PA config
...
static void nus_data_handler(ble_nus_evt_t *p_evt) {
  ...
  if (p_evt->type == BLE_NUS_EVT_RX_DATA) {
    ...
    if ((p_evt->params.rx_data.p_data[0] == '0') &&
        (p_evt->params.rx_data.p_data[1] == '1')) {
    ...
    if ((p_evt->params.rx_data.p_data[0] == '0') &&
        (p_evt->params.rx_data.p_data[1] == '2')) {

      sleep_in_time_ticker =
          (uint32_t)(((p_evt->params.rx_data.p_data[2] - 0x30) * 10000) +
                     ((p_evt->params.rx_data.p_data[3] - 0x30) * 1000) +
                     ((p_evt->params.rx_data.p_data[4] - 0x30) * 100) +
                     ((p_evt->params.rx_data.p_data[5] - 0x30) * 10) +
                     ((p_evt->params.rx_data.p_data[6] - 0x30) * 1)) *
          10; // de segundos se pasa a ticker de 0.1 segundos, se multiplica por
              // 10
      NRF_LOG_RAW_INFO("Tiempo de dormido (02) a %i\r\n", sleep_in_time_ticker);
      NRF_LOG_FLUSH();
      if (sleep_in_time_ticker >= 20 &&
          sleep_in_time_ticker <=
              144000) // Permite dormido entre 1 segundo y 4 horas
      {
    ...
    if ((p_evt->params.rx_data.p_data[0] == '0') &&
        (p_evt->params.rx_data.p_data[1] == '3')) {
      ...
      APP_ADV_DURATION =
          ...
          100; // de segundos se pasa a ticker de 0.1 segundos, se multiplica
               // por 10
      NRF_LOG_RAW_INFO(
          "Comando 03 recepcionado : Modificando tiempo de adv a %i\r\n",
          APP_ADV_DURATION);
      NRF_LOG_FLUSH();

      if (APP_ADV_DURATION >= 100 &&
          APP_ADV_DURATION <=
              6000) // Permite adverting entre 1 segundo y 60 segundos
      {
    ...
    if ((p_evt->params.rx_data.p_data[0] == '0') &&
        (p_evt->params.rx_data.p_data[1] == '4')) {
    ...
    // 05 Resetea El nodo
    if ((p_evt->params.rx_data.p_data[0] == '0') &&
        (p_evt->params.rx_data.p_data[1] == '5')) {
    ...
    if ((p_evt->params.rx_data.p_data[0] == '0') &&
        (p_evt->params.rx_data.p_data[1] == '6')) {
      ...
      // YYYY MM DD HH MM SS
      t.year = (d[2] - '0') * 1000 + (d[3] - '0') * 100 + (d[4] - '0') * 10 +
               (d[5] - '0');
      ...
      // Log formateado
      NRF_LOG_RAW_INFO("%04u/%02u/%02u   ", Flash_array.year, Flash_array.month,
                       Flash_array.date);
      NRF_LOG_RAW_INFO("%02u:%02u:%02u\r\n", Flash_array.hour,
                       Flash_array.minute, Flash_array.second);
      NRF_LOG_FLUSH();
    ...
    if ((p_evt->params.rx_data.p_data[0] == '0') &&
      ...
      data_arr[7] =
          (char)(((t.year / 100) - (int)((t.year / 1000) * 10)) + 0x30);
      ...
      if ((err_code != NRF_ERROR_INVALID_STATE) &&
          (err_code != NRF_ERROR_BUSY) && (err_code != NRF_ERROR_NOT_FOUND)) {
        APP_ERROR_CHECK(err_code);
      }
    }
    ...
    if ((p_evt->params.rx_data.p_data[0] == '0') &&
        (p_evt->params.rx_data.p_data[1] == '8')) {
    ...
    if ((p_evt->params.rx_data.p_data[0] == '0') &&
        (p_evt->params.rx_data.p_data[1] == '9')) {
    ...
    // 10 Modificar el tiempo de encendido y dormido
    // Formato Comando: 10 + Tiempo encendido 00010 en segundos + Tiempo de
    // dormido 00010 en segundos
    // if ((p_evt->parmas.rx_data.p_data[0] == '1') &&
    // (p_evt->params.rx_data.p_data[1] == '0'))
    //{
    //  NRF_LOG_RAW_INFO("\nSe cambiara la hora de encendido y dormido");

    //  // Tiempo de dormido
    //  Flash_array.sleep_time[0] = (p_evt->params.rx_data.p_data[2] - 0x30);
    //  Flash_array.sleep_time[1] = (p_evt->params.rx_data.p_data[3] - 0x30);
    //  Flash_array.sleep_time[2] = (p_evt->params.rx_data.p_data[4)])

    //  // Tiempo de encendido
    //  Flash_array.adv_time[]

    //}

    // 20 modifica la MAC 20AABBCCDDEEFF
    if ((p_evt->params.rx_data.p_data[0] == '2') &&
        (p_evt->params.rx_data.p_data[1] == '0')) {
    ...
    if ((p_evt->params.rx_data.p_data[0] == '5') &&
        (p_evt->params.rx_data.p_data[1] == '0')) {
...
void Next_Transmition() {
  ...
  if (Tipo_Envio == History_By_Index) {
    ...
    if (Requested_History_Index < Size_Memory_History) {
      ...
      data_array[position++] =
          0xA8; // Código para identificar el envío por índice
  ...
  if (Tipo_Envio == Last_History) {
    ...
    data_array[position++] =
        0x98; // Código para identificar el envío del último historial
...
static void advertising_init(void) {
  ...
  nrf_delay_ms(500);
  Battery_level = 0;
  Battery_level = GET_MEASURE_SENSOR(
      ADC_Battery, Gain_Battery); // Circuito_List[0]  NRF_SAADC_INPUT_AIN2
  ...
  uint16_t Pista1 =
      GET_MEASURE_SENSOR(Circuito_1, Gain_Circuit); // Circuito_List[0]
  if (Pista1 > 0xf339)
    Pista1 = 0x0;
  NRF_LOG_RAW_INFO("valor circuito 1 %d \r\n", Pista1);
  NRF_LOG_FLUSH();
  uint16_t Pista2 = GET_MEASURE_SENSOR(
      Circuito_2, Gain_Circuit); // Circuito_List[0]  NRF_SAADC_INPUT_AIN2
  if (Pista2 > 0xf339)
    Pista2 = 0x0;
  ...
  m_beacon_info[12] =
      Flash_array.total_reset[3]; // Imprimiremos momentaneamente la cantidad de
                                  // reset del equipo
