...
#define APP_ADV_INTERVAL                200                                         /**< The advertising interval (in units of 0.625 ms. This value corresponds to 40 ms). antes 64*/  //antes 90 
...
#define COMPARE_COUNTERTIME             (2UL)                                       //3UL   este dato es la cantidad de segundos de seteo del comparador RTC.
...
int8_t          Potencia_antenna                   = -20; //4                        // Radio transmit power in dBm (accepted values are -40, -30, -20, -16, -12, -8, -4, 0, 3, and 4 dBm). 4 maximo para nrf52832
int16_t         Max_Time_Unconfident               = 400;                       // Tiempo maximo de conexion establecida sin asegurar fuente confiable, 40 segundos.
...
uint32_t        APP_ADV_DURATION                   = 500;                      /**< 1500 son 15 segundos The advertising duration (180 seconds) in units of 10 milliseconds.  era 800 */  
...
uint8_t         Offset_desgaste                    = 0x00;                      // valor que corrige la distancia entre el perno y el inicio de la placa.
int8_t          Offset_sensor                      = 0x00;                      // valor que corrige cuando se corta un sensor, es decir se corta para adaptarlo a un perno.
...
0x4,0x64,0x6A,                  // Sleep Time  in mSecond         Comienza en 17          4646A son 8 horas
0x5,0xdc,                       // ADV Time in mSecond            Comienza en 20
0x0,0x0,0x0,0x0,0x0,0x0,        // MAC de fabrica                 Comienza en 22 
0x00,                           // Tipo 00 6.2Kohm 01 6.8Kohm     Comienza en 28          Para corregir el error.
...
0x80,                            // creacion de offset de desgaste Comienza en 31      el cero es 128 de valor     
